(function ( $ ) {
    ...
    $.fn.richText = function( options ) {
        ...
        var $dropdownList = $('<ul />', {class: "richText-dropdown"}), // dropdown lists
            ...
            $formButton = $('<button />', {text: "Wykonaj", class: "btn"}); // button
        ...
        $linksForm.append(
            $formItem.clone()
                .append($formLabel.clone().text("Tekst").attr("for", "urlText"))
                .append($formInput.clone().attr("id", "urlText"))
               );
        $linksForm.append(
            $formItem.clone()
                .append($formLabel.clone().text("Otwórz w").attr("for", "openIn"))
                .append(
                    $formInputSelect
                        .clone().attr("id", "openIn")
                        .append($("<option />", {value: '_self', text: 'Ta samo okno'}))
                        .append($("<option />", {value: '_blank', text: 'Nowe okno'}))
                    )
               );
        ...
        $videoForm.append(
            $formItem.clone()
                .append($formLabel.clone().text("Wielkość").attr("for", "size"))
                .append(
                        $formInputSelect
                            .clone().attr("id", "size")
                            .append($("<option />", {value: 'responsive', text: 'Responsywne'}))
        ...
        if(settings.imageHTML 
            ...
            $imageForm.append(
                $formItem.clone()
                    .append($formLabel.clone().text("URL zdjęcia").attr("for", "imageURL"))
                    .append($formInput.clone().attr("id", "imageURL"))
                   );
            $imageForm.append(
                $formItem.clone()
                    .append($formLabel.clone().text("Wyrównaj").attr("for", "align"))
                    .append(
                        $formInputSelect
                            .clone().attr("id", "align")
                            .append($("<option />", {value: 'left', text: 'Lewo'}))
                            .append($("<option />", {value: 'center', text: 'Środek'}))
                            .append($("<option />", {value: 'right', text: 'Prawo'}))
                        )
                   );
        }
        ...
        if(settings.fileHTML 
            ...
            $fileForm.append(
                $formItem.clone()
                    .append($formLabel.clone().text("URL pliku").attr("for", "fileURL"))
                    .append($formInput.clone().attr("id", "fileURL"))
                );
            $fileForm.append(
                $formItem.clone()
                    .append($formLabel.clone().text("Tekst linka").attr("for", "fileText"))
        ...
        $tableForm.append(
            $formItem.clone()
                .append($formLabel.clone().text("Wiersze").attr("for", "tableRows"))
                .append($formInput.clone().attr("id", "tableRows").attr("type", "number"))
            );
        $tableForm.append(
            $formItem.clone()
                .append($formLabel.clone().text("Kolumny").attr("for", "tableColumns"))
        ...
        $(document).on("click", "#richText-Video button.btn", function(event) {
            ...
            if($form.attr("data-editor") === editorID) {
                ...
                if(!url) {
                    // no url set
                    $form.prepend($('<div />', {style: 'color:red;display:none;', class: 'form-item is-error', text: 'Proszę wpisać URL'}));
        ...
        $(document).on("click", "#richText-Image button.btn", function(event) {
            ...
            if($form.attr("data-editor") === editorID) {
                ...
                if(!url) {
                    // no url set
                    $form.prepend($('<div />', {style: 'color:red;display:none;', class: 'form-item is-error', text: 'Proszę wybrać zdjęcie.'}));
                    $form.children('.form-item.is-error').slideDown();
                    setTimeout(function() {
                        $form.children('.form-item.is-error').slideUp(function () {
                            $(this).remove();
        ...
        $(document).on("click", "#richText-File button.btn", function(event) {
            ...
            if($form.attr("data-editor") === editorID) {
                ...
                if(!url) {
                    // no url set
                    $form.prepend($('<div />', {style: 'color:red;display:none;', class: 'form-item is-error', text: 'Proszę wybrać plik.'}));
                    $form.children('.form-item.is-error').slideDown();
                    setTimeout(function() {
                        $form.children('.form-item.is-error').slideUp(function () {
                            $(this).remove();
